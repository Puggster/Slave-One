...
republicEisleyInvasion = ScreenPlay:new {
	scriptName = "republicEisleyInvasion",
	locations = {
		{
			planetName = "tatooine",
			x = 3611,
...
registerScreenPlay("republicEisleyInvasion", false)


function republicEisleyInvasion:start()
...
function republicEisleyInvasion:stop()
	self:cleanupScene()
end

function republicEisleyInvasion:cleanupScene()
	local spawnedObjects = readStringData("republicEisleyInvasion:spawnedObjects")
	...
	deleteStringData("republicEisleyInvasion:spawnedObjects")
	deleteData("republicEisleyInvasion:mobileID")
	deleteData("republicEisleyInvasion:mobNumber")
	deleteData("republicEisleyInvasion:numDestroyed")
end

function republicEisleyInvasion:getSpawnInRadius(center, radius)
...
function republicEisleyInvasion:spawnSingleMobile(none, num, mobileName, location)
  local pMobile
	...
	if (spawnPoint ~= nil) then
		...
		writeData("republicEisleyInvasion:mobileID" .. num, SceneObject(pMobile):getObjectID())
		writeData("republicEisleyInvasion:mobNumber" .. SceneObject(pMobile):getObjectID(), num)
...
function republicEisleyInvasion:spawnMobile(none, num, mobileList, location)
...
function republicEisleyInvasion:notifyMobileDestroyed(pMobile, pAttacker)
	if (pMobile == nil or pAttacker == nil) then
		return 0
	end

	local numDestroyed = readData("republicEisleyInvasion:numDestroyed")
	numDestroyed = numDestroyed + 1
	writeData("republicEisleyInvasion:numDestroyed", numDestroyed)
	if(numDestroyed == (self.unitSpawnCount + self.toughUnitSpawnCount + #self.requiredMobiles) * #self.locations) then
		print("Screenplay Ended -- Cleanup in 10s -- republicEisleyInvasion")
...
function republicEisleyInvasion:storeObjectID(pObject)
	if (pObject == nil) then
		return
	end
	local objectID = SceneObject(pObject):getObjectID()
	writeStringData("republicEisleyInvasion:spawnedObjects", readStringData("republicEisleyInvasion:spawnedObjects") .. "," .. objectID)
end

function republicEisleyInvasion:destroyObject(pObject)
	if (pObject ~= nil) then
		if (SceneObject(pObject):isAiAgent()) then
			CreatureObject(pObject):setPvpStatusBitmask(0)
			forcePeace(pObject)
