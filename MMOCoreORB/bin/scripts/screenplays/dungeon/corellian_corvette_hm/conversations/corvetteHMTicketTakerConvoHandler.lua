...
CorvetteHMTicketTakerConvoHandler = conv_handler:new {
	ticketTaker = nil
}

function CorvetteHMTicketTakerConvoHandler:setTicketTaker(taker)
	self.ticketTaker = taker
end

function CorvetteHMTicketTakerConvoHandler:runScreenHandlers(pConvTemplate, pPlayer, pNpc, selectedOption, pConvScreen)
	...
	if (screenID == "start") then
		if (activeStep == 2) then
			if (self.ticketTaker.faction == 0) then
				if (CreatureObject(pPlayer):getGroupSize() > 5) then
				...
				if (CreatureObject(pPlayer):getGroupSize() > 5) and (rightFaction == false) then
					clonedConversation:addOption(self.ticketTaker.aboutMissionString, "mission_wrong_faction_too_many")
				elseif (CreatureObject(pPlayer):getGroupSize() > 5) then
...
function CorvetteHMTicketTakerConvoHandler:getInitialScreen(pPlayer, pNpc, pConvTemplate)
