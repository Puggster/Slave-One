...
function CorvetteRoomPanelMenuComponent:handleObjectMenuSelect(pSceneObject, pPlayer, selectedID)
	...
	if (roomName == "meetingroom38") then
		...
		CorellianCorvetteHM:unlockRoom(pCorvetteHM, "meetingroom38")
	...
	elseif (roomName == "elevator57") then
		...
		CorellianCorvetteHM:unlockRoom(pCorvetteHM, "elevator57")
...
function CorvetteDestroyTerminalMenuComponent:handleObjectMenuSelect(pSceneObject, pPlayer, selectedID)
	...
	local corvetteHMID = readData(SceneObject(pPlayer):getObjectID() .. "corvetteHMID")
	local questType = readStringData("corvetteQuestType:" .. corvetteHMID)
	...
	if (terminalType == "engine") then
		CorvetteHMSui:sendEngineSui(pPlayer, pSceneObject)
	elseif (terminalType == "fuel") then
		CorvetteHMSui:sendFuelSui(pPlayer, pSceneObject)
	elseif (terminalType == "hyperdrive") then
		CorvetteHMSui:sendHyperdriveSui(pPlayer, pSceneObject)
	end
end

CorvetteHMKeypadMenuComponent = { }

function CorvetteHMKeypadMenuComponent:fillObjectMenuResponse(pKeypad, pMenuResponse, pPlayer)
end

function CorvetteHMKeypadMenuComponent:handleObjectMenuSelect(pKeypad, pPlayer, selectedID)
	...
	if (keypadRoom == "elevator57") then
		if (CorellianCorvetteHM:readDataFromGroup(pCorvetteHM, ":unlocked:elevator57") == 1) then
			CreatureObject(pPlayer):sendSystemMessage("@dungeon/corvette:open") -- This door is now unlocked.
			CorellianCorvetteHM:unlockRoom(pCorvetteHM, "elevator57")
			CorellianCorvetteHM:writeDataToGroup(pCorvetteHM, ":unlocked:elevator57", 1)
		else
	...
	elseif (keypadRoom == "meetingroom38") then
		if (CorellianCorvetteHM:readDataFromGroup(pCorvetteHM, ":unlocked:meetingroom38") == 1) then
			CreatureObject(pPlayer):sendSystemMessage("@dungeon/corvette:open") -- This door is now unlocked.
			CorellianCorvetteHM:unlockRoom(pCorvetteHM, "meetingroom38")
			CorellianCorvetteHM:writeDataToGroup(pCorvetteHM, ":unlocked:meetingroom38", 1)
		else
	...
	elseif (keypadRoom ~= "") then
		CorvetteHMSui:sendKeypadSui(pKeypad, pPlayer)
	end

end

CorvetteHMComputerMenuComponent = { }

function CorvetteHMComputerMenuComponent:fillObjectMenuResponse(pComputer, pMenuResponse, pPlayer)
end

function CorvetteHMComputerMenuComponent:handleObjectMenuSelect(pComputer, pPlayer, selectedID)
	...
	if (readData(computerID .. ":spawnedEnemies") ~= 1) then
		CorellianCorvetteHM:spawnComputerEnemies(pCorvetteHM, pComputer)
	...
	elseif (computerLabel == "two") then
		...
		if (accessCode == 0) then
			...
			CorellianCorvetteHM:writeDataToGroup(pCorvetteHM, ":code:armorybackroom55", accessCode)
	...
	elseif (computerLabel == "four") then
		if (readData(computerID .. ":terminalRebooted") == 1) then
			...
			if (accessCode == 0) then
				accessCode = getRandomNumber(10000, 99999)
				CorellianCorvetteHM:writeDataToGroup(pCorvetteHM, ":code:bridge66", accessCode)
				writeData(computerID .. ":accessCode", accessCode)
			end
			...
			if (pInventory ~= nil) then
				...
				if (pBootDisk ~= nil) then
					...
					local slicingSkill = CorellianCorvetteHM:checkSlicingSkill(pPlayer)
	...
	elseif (computerLabel == "six") then
		...
		if (accessCode == 0) then
			accessCode = getRandomNumber(10000, 99999)
			CorellianCorvetteHM:writeDataToGroup(pCorvetteHM, ":code:officerquarters64", accessCode)
			writeData(computerID .. ":accessCode", accessCode)
		end
	...
	elseif (computerLabel == "seven") then
		...
		if (accessCode == 0) then
			accessCode = getRandomNumber(10000, 99999)
			CorellianCorvetteHM:writeDataToGroup(pCorvetteHM, ":code:officerquarters65", accessCode)
			writeData(computerID .. ":accessCode", accessCode)
		end
	...
	elseif (computerLabel == "eight") then
		...
		if (accessCode == 0) then
			accessCode = getRandomNumber(10000, 99999)
			CorellianCorvetteHM:writeDataToGroup(pCorvetteHM, ":code:officerquarters63", accessCode)
			writeData(computerID .. ":accessCode", accessCode)
		end
