CorvetteHMSui = ScreenPlay:new {}

function CorvetteHMSui:sendEngineSui(pPlayer, pSceneObject)
	local sui = SuiMessageBox.new("CorvetteHMSui", "engineSuiCallback")
...
function CorvetteHMSui:engineSuiCallback(pPlayer, pSui, eventIndex, ...)
	...
	if (readData(corvetteHMID .. ":engineDestroyed") == 1) then
	...
	if (cancelPressed) then -- B
		pickedValue = valueB
		writeData(corvetteHMID .. ":engineSetting", valueB)
	elseif (revertPressed ~= nil) then -- A
		pickedValue = valueA
		writeData(corvetteHMID .. ":engineSetting", valueA)
	elseif (eventIndex == 0) then -- C
		pickedValue = valueC
		writeData(corvetteHMID .. ":engineSetting", valueC)
	end
	...
	local fuelSetting = readData(corvetteHMID .. ":fuelSetting")
	local hyperdriveSetting = readData(corvetteHMID .. ":hyperdriveSetting")
	...
	CreatureObject(pPlayer):sendSystemMessage("@dungeon/corvette:blew_up") -- You have successfully managed to destroy the engines! Your mission is complete.
	writeData(corvetteHMID .. ":engineDestroyed", 1)
	CorellianCorvetteHM:handleQuestSuccess(pCorvetteHM)
end

function CorvetteHMSui:sendFuelSui(pPlayer, pSceneObject)
	local sui = SuiMessageBox.new("CorvetteHMSui", "fuelSuiCallback")
...
function CorvetteHMSui:fuelSuiCallback(pPlayer, pSui, eventIndex, ...)
	if (pPlayer == nil) then
		return
	end
	...
	local fuelSetting = 0
	local hyperdriveSetting = readData(corvetteHMID .. ":hyperdriveSetting")
	local engineSetting = readData(corvetteHMID .. ":engineSetting")
	...
	writeData(corvetteHMID .. ":fuelSetting", fuelSetting)
...
function CorvetteHMSui:sendHyperdriveSui(pPlayer, pSceneObject)
	local sui = SuiMessageBox.new("CorvetteHMSui", "hyperdriveSuiCallback")
...
function CorvetteHMSui:hyperdriveSuiCallback(pPlayer, pSui, eventIndex, ...)
	if (pPlayer == nil) then
		return
	end
	...
	writeData(corvetteHMID .. ":hyperdriveSetting", powerSetting)
...
function CorvetteHMSui:sendKeypadSui(pKeypad, pPlayer)
	local sui = SuiKeypad.new("CorvetteHMSui", "keypadSuiCallback")
...
function CorvetteHMSui:keypadSuiCallback(pPlayer, pSui, eventIndex, args)
	if (pPlayer == nil) then
		return
	end
	...
	local keypadCode = CorellianCorvetteHM:readDataFromGroup(pCorvetteHM, ":code:" .. keypadRoom)
	local enteredCode = 0
	...
	if (keypadCode ~= nil and keypadCode ~= "" and enteredCode == keypadCode) then
		...
		CorellianCorvetteHM:unlockRoom(pCorvetteHM, keypadRoom)
	else
