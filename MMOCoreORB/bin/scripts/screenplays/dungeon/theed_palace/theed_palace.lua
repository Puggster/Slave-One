...
registerScreenPlay("TheedPalace", true)

function TheedPalace:start()
	if (isZoneEnabled("dungeon3")) then
		self:initialize()
	end
end

function TheedPalace:initialize()
	local num = 0
	for i = 1, #self.buildings, 1 do
		local building = self.buildings[i]
		for j = 1, #building.buildingIds, 1 do
			local pPalace = getSceneObject(building.buildingIds[j])
			if pPalace == nil or not SceneObject(pPalace):isBuildingObject() then
				printLuaError("TheedPalace:initialize tried using a corvette id that was nil or not a building: " .. building.buildingIds[j])
			else
				local palaceID = SceneObject(pPalace):getObjectID()
				deleteData("corvetteActive:" .. palaceID)
				self:ejectAllPlayers(pPalace)
				deleteData("corvettePlayerCount:" .. palaceID)
				createObserver(ENTEREDBUILDING, "TheedPalace", "onEnterCorvette", pPalace)
				createObserver(EXITEDBUILDING, "TheedPalace", "onExitCorvette", pPalace)
				num = num + 1
			end
		end
	end
...
function TheedPalace:activate(pPlayer, faction, questType)
	...
	for i = 1, #ids, 1 do
		...
		if (active == 1) then
			...
			if (timeLeftSecs < 0) then
				local pPalace = getSceneObject(ids[i])

				if (pPalace ~= nil) then
					self:ejectAllPlayers(pPalace)
					createEvent(5000, "TheedPalace", "doCorvetteCleanup", pPalace, "")
				end
			end
		else
			palaceID = ids[i]
	...
	local pPalace = getSceneObject(palaceID)

	if (pPalace == nil) then
		CreatureObject(pPlayer):sendSystemMessage("@dungeon/space_dungeon:corellian_corvette_travel_fail")
		printLuaError("TheedPalace:activate unable to find corvette object with id " .. palaceID)
		return false
	end

	self:startQuest(pPalace, questType)

	local playerID = SceneObject(pPlayer):getObjectID()
	writeData(playerID .. "palaceID", palaceID)
	writeData(palaceID .. ":ownerID", playerID)
	createEvent(1000, "TheedPalace", "transportPlayer", pPlayer, "")
	...
	if (CreatureObject(pPlayer):isGrouped()) then
		...
		for i = 0, groupSize - 1, 1 do
			...
			if pMember ~= nil and pMember ~= pPlayer and CreatureObject(pPlayer):isInRangeWithObject(pMember, 50) and not SceneObject(pMember):isAiAgent() then
				self:sendAuthorizationSui(pMember, pPlayer, pPalace)
			end
		end
	end
...
function TheedPalace:getFactionCRC(faction)
...
function TheedPalace:sendAuthorizationSui(pPlayer, pLeader, pPalace)
	if (pPlayer == nil or pPalace == nil) then
		return
	end

	local corvetteFaction = self:getBuildingFaction(pPalace)
	...
	writeData(SceneObject(pPlayer):getObjectID() .. "palaceID", SceneObject(pPalace):getObjectID())

	local sui = SuiMessageBox.new("TheedPalace", "authorizationSuiCallback")
	sui.setTargetNetworkId(SceneObject(pPalace):getObjectID())
	local corvetteName = getStringId("@dungeon/space_dungeon:corvette_" .. self:getBuildingFaction(pPalace))
	...
	createEvent(30 * 1000, "TheedPalace", "closeAuthorizationSui", pPlayer, pageId)
end

function TheedPalace:authorizationSuiCallback(pPlayer, pSui, eventIndex, args)
	local cancelPressed = (eventIndex == 1)

	if (cancelPressed) then
		deleteData(SceneObject(pPlayer):getObjectID() .. "palaceID")
		return
	end
...
function TheedPalace:closeAuthorizationSui(pPlayer, pageId)
	local pPalace = self:getCorvetteObject(pPlayer)
	local playerID = SceneObject(pPlayer):getObjectID()
	local pageId = tonumber(pageId)

	if (pPalace ~= nil and SceneObject(pPalace):getObjectID() == readData(playerID .. "palaceID")) then
		return
	end
	...
	deleteData(playerID .. "palaceID")
end

function TheedPalace:lockRooms(pPalace)
	if (pPalace == nil) then
		return
	end
	...
	for i = 1 ,#self.lockedRooms, 1 do
		local pCell = BuildingObject(pPalace):getNamedCell(self.lockedRooms[i])
...
function TheedPalace:unlockRoom(pPalace, roomName)
	if (pPalace == nil) then
		return
	end
	...
	local pCell = BuildingObject(pPalace):getNamedCell(roomName)
	...
	if (pCell ~= nil) then
		...
		BuildingObject(pPalace):broadcastSpecificCellPermissions(cellID)
	end
end

function TheedPalace:setupSceneObjects(pPalace)
	if (pPalace == nil) then
		return
	end
	...
	if (readData("corvetteActive:" .. palaceID) ~= 1) then
		return
	end

	for i = 1, #palaceStaticSpawns, 1 do
		local spawnData = palaceStaticSpawns[i]
		local pCell = BuildingObject(pPalace):getNamedCell(spawnData[5])
		...
		if (pCell ~= nil) then
			if (spawnData[10] == nil or spawnData[10] == "" or spawnData[10] == corvetteFaction) then
				...
				if (pObject == nil) then
					printLuaError("TheedPalace:setupSceneObjects unable to spawn scene object " .. spawnData[1] .. " for corvette ID " .. palaceID)
				else
					...
					if (spawnData[8] ~= nil and spawnData[8] ~= "") then
						...
						createEvent(100, "TheedPalace", spawnData[8], pObject, extraData)
					end
				end
			end
		else
...
function TheedPalace:spawnNpcs(pPalace)
	if (pPalace == nil) then
		return
	end
	...
	local spawnTable = palaceNeutralSpawns

	if (corvetteFaction == "imperial") then
		spawnTable = palaceImperialSpawns
	elseif (corvetteFaction == "rebel") then
		spawnTable = palaceRebelSpawns
	end

	for i = 1, #spawnTable, 1 do
		local spawnData = spawnTable[i]
		local pCell = BuildingObject(pPalace):getNamedCell(spawnData[6])
		...
		if (pCell ~= nil) then
			...
			if (pMobile == nil) then
				printLuaError("TheedPalace:setupSceneObjects unable to spawn mobile " .. spawnData[1] .. " for " .. corvetteFaction .. " corvette ID " .. palaceID)
			else
				CreatureObject(pMobile):setOptionBit(IGNORE_FACTION_STANDING)
				if (spawnData[7] ~= nil and spawnData[7] ~= "") then
					createEvent(100, "TheedPalace", spawnData[7], pMobile, "")
				end
			end
		else
			printLuaError("Unable to grab cell object for cell named " .. spawnData[6] .. " in corvette ID " .. palaceID .. " when trying to spawn " .. spawnData[1])
		end
	end
end

function TheedPalace:setupAssassinationTarget(pTarget)
	if (pTarget == nil) then
		return
	end

	local pPalace = self:getCorvetteObject(pTarget)
	...
	createObserver(OBJECTDESTRUCTION, "TheedPalace", "onAssassinationTargetKilled", pTarget)
end

function TheedPalace:onAssassinationTargetKilled(pTarget, pKiller)
	...
	local questType = readStringData("corvetteQuestType:" .. SceneObject(pPalace):getObjectID())

	if (questType ~= "assassinate") then
		return 1
	end

	TheedPalace:handleQuestSuccess(pPalace)

	return 1
end

function TheedPalace:setupBrokenDroid(pDroid)
	if (pDroid == nil) then
		return
	end

	local pPalace = self:getCorvetteObject(pDroid)
	...
	local pCell = BuildingObject(pPalace):getNamedCell("thrustersubroom28")
	...
	writeData(palaceID .. ":H3P0ID", SceneObject(pH3P0):getObjectID())
	...
	for i = 1, #self.electricTrapLocs, 1 do
		...
		if pTrap ~= nil then
			...
			writeData(palaceID .. ":electricTrap" .. i, trapID)
			...
			if pTrapArea ~= nil then
				...
				createObserver(ENTEREDAREA, "TheedPalace", "notifyTrapAreaEntered", pTrapArea)
			end
		end
	end

	createObserver(DESTINATIONREACHED, "TheedPalace", "repairDroidDestinationReached", pDroid)
	...
	writeData(palaceID .. ":electricTrapEnabled", 1)
end

function TheedPalace:startRepairDroidMovement(pDroid)
	local pPalace = self:getCorvetteObject(pDroid)
...
function TheedPalace:repairDroidDestinationReached(pDroid)
	if (pDroid == nil) then
		return 1
	end

	createEvent(3 * 1000, "TheedPalace", "startRepairDroidRepairing", pDroid, "")

	return 1
end

function TheedPalace:startRepairDroidRepairing(pDroid)
	local pPalace = self:getCorvetteObject(pDroid)
	...
	local palaceID = BuildingObject(pPalace):getObjectID()
	CreatureObject(pDroid):doAnimation("sp_10")

	createEvent(3 * 1000, "TheedPalace", "removeElectricTrap", pPalace, "")
end

function TheedPalace:removeElectricTrap(pPalace)
	if (pPalace == nil) then
		return
	end
	...
	for i = 1, #self.electricTrapLocs, 1 do
		local trapID = readData(palaceID .. ":electricTrap" .. i)
		...
		deleteData(palaceID .. ":electricTrap" .. i)
	end

	local H3POID = readData(palaceID .. ":H3P0ID")
	local pH3PO = getSceneObject(H3POID)

	if (pH3PO ~= nil) then
		spatialChat(pH3PO, "@dungeon/corvette:pdroid_congrats")
		writeData(palaceID.. ":repairDroidComplete", 1)
	end
end

function TheedPalace:notifyTrapAreaEntered(pActiveArea, pMovingObject)
	...
	local pPalace = self:getCorvetteObject(pMovingObject)

	if (pPalace == nil) then
		return 1
	end

	if (readData(SceneObject(pPalace):getObjectID() .. ":electricTrapEnabled") ~= 1) then
		return 0
	end
...
function TheedPalace:setupPrisoner(pPrisoner)
	local pPalace = self:getCorvetteObject(pPrisoner)

	if (pPalace == nil) then
		return
	end

	createObserver(DESTINATIONREACHED, "TheedPalace", "prisonerDestinationReached", pPrisoner)
...
function TheedPalace:doPrisonerEscape(pPrisoner)
	local pPalace = self:getCorvetteObject(pPrisoner)
	...
	local pCell = BuildingObject(pPalace):getNamedCell("elevator57")

	if (pCell == nil) then
		return
	end

	createEvent(60 * 1000, "TheedPalace", "removePrisoner", pPrisoner, "")
...
function TheedPalace:prisonerDestinationReached(pPrisoner)
	if (pPrisoner == nil) then
		return 1
	end

	createEvent(2 * 1000, "TheedPalace", "removePrisoner", pPrisoner, "")

	return 1
end

function TheedPalace:removePrisoner(pPrisoner)
	if (pPrisoner == nil) then
		return
	end
...
function TheedPalace:setupRoomPanel(pPanel, room)
	if (pPanel == nil or room == "") then
		return
	end

	SceneObject(pPanel):setObjectMenuComponent("CorvetteRoomPanelMenuComponent")
	createObserver(OBJECTREMOVEDFROMZONE, "TheedPalace", "notifyRoomPanelRemovedFromZone", pPanel)
	writeStringData(SceneObject(pPanel):getObjectID() .. ":panelRoom", room)
end

function TheedPalace:notifyRoomPanelRemovedFromZone(pPanel)
	if (pPanel == nil) then
		return 1
	end
...
function TheedPalace:setupKeypad(pKeypad, room)
	if (pKeypad == nil or room == "") then
		return
	end

	SceneObject(pKeypad):setObjectMenuComponent("CorvetteKeypadMenuComponent")
	createObserver(OBJECTREMOVEDFROMZONE, "TheedPalace", "notifyKeypadRemovedFromZone", pKeypad)
	writeStringData(SceneObject(pKeypad):getObjectID() .. ":keypadRoom", room)
end

function TheedPalace:notifyKeypadRemovedFromZone(pKeypad)
	if (pKeypad == nil) then
		return 1
	end
...
function TheedPalace:setupDestroyTerminal(pTerminal, type)
	if (pTerminal == nil or type == "") then
		return
	end

	SceneObject(pTerminal):setObjectMenuComponent("CorvetteDestroyTerminalMenuComponent")
	createObserver(OBJECTREMOVEDFROMZONE, "TheedPalace", "notifyDestroyTerminalRemovedFromZone", pTerminal)
	writeStringData(SceneObject(pTerminal):getObjectID() .. ":terminalType", type)
end

function TheedPalace:notifyDestroyTerminalRemovedFromZone(pTerminal)
...
function TheedPalace:setupComputerObject(pComputer, label)
	if (pComputer == nil or label == "") then
		return
	end

	SceneObject(pComputer):setObjectMenuComponent("CorvetteComputerMenuComponent")
	createObserver(OBJECTREMOVEDFROMZONE, "TheedPalace", "notifyComputerRemovedFromZone", pComputer)
	writeStringData(SceneObject(pComputer):getObjectID() .. ":computerLabel", label)
end

function TheedPalace:spawnComputerEnemies(pPalace, pComputer)
	if (pPalace == nil or pComputer == nil) then
		return
	end
...
function TheedPalace:notifyComputerRemovedFromZone(pComputer)
...
function TheedPalace:setupEscapePod(pPod)
	if (pPod == nil) then
		return
	end

	createObserver(OBJECTRADIALUSED, "TheedPalace", "notifyPodRadialUsed", pPod)
	createObserver(OBJECTREMOVEDFROMZONE, "TheedPalace", "notifyPodRemovedFromZone", pPod)
	...
	if pActiveArea ~= nil then
		createObserver(ENTEREDAREA, "TheedPalace", "notifyEnteredEscapePodArea", pActiveArea)
		writeData(SceneObject(pPod):getObjectID() .. ":areaID", SceneObject(pActiveArea):getObjectID())
	end
end

function TheedPalace:notifyPodRemovedFromZone(pPod)
	if (pPod == nil) then
		return 1
	end
...
function TheedPalace:notifyPodRadialUsed(pPod, pPlayer, radialSelected)
	if (pPod == nil) then
		return 1
	end
	...
	if (radialSelected == 20) then
		createEvent(2 * 1000, "TheedPalace", "ejectPlayer", pPlayer,"")
		return 0
	end
...
function TheedPalace:notifyEnteredEscapePodArea(pActiveArea, pPlayer)
	if (pActiveArea == nil) then
		return 1
	end
...
function TheedPalace:setupLootCrate(pCrate, type)
	if (pCrate == nil) then
		return
	end

	local pPalace = self:getCorvetteObject(pCrate)

	if (pPalace == nil) then
		return
	end

	local faction = self:getBuildingFaction(pPalace)
	...
	createObserver(OBJECTREMOVEDFROMZONE, "TheedPalace", "notifyLootCrateRemovedFromZone", pCrate)
	SceneObject(pCrate):setContainerComponent("corvetteLootCrateContainerComponent")
end

function TheedPalace:onCrateLooted(pCrate)
	if (pCrate == nil) then
		return
	end
	...
	if (crateType == "r2") then
		createEvent(600 * 1000, "TheedPalace", "setupLootCrate", pCrate, "r2")
...
function TheedPalace:notifyLootCrateRemovedFromZone(pCrate)
	if (pCrate == nil) then
		return 1
	end
...
function TheedPalace:transportPlayer(pPlayer)
	if pPlayer == nil then
		return
	end
	...
	local palaceID = readData(SceneObject(pPlayer):getObjectID() .. "palaceID")

	local pPalace = getSceneObject(palaceID)

	if (pPalace == nil) then
		printLuaError("TheedPalace:transportPlayer nil corvette object using corvette id " .. palaceID)
		return
	end
	...
	local pCell = BuildingObject(pPalace):getCell(1)
...
function TheedPalace:startQuest(pPalace, questType)
	local dungeonID = self:getNewDungeonID()
	local palaceID = SceneObject(pPalace):getObjectID()
	writeData("corvetteDungeonID:" .. palaceID, dungeonID)
	writeData("corvetteActive:" .. palaceID, 1)
	writeData("corvetteStartTime:" .. palaceID, os.time())
	writeStringData("corvetteQuestType:" .. palaceID, questType)
	createEvent(5 * 60 * 1000, "TheedPalace", "handleCorvetteTimer", pPalace, "")

	self:setupSceneObjects(pPalace)
	self:spawnNpcs(pPalace)
	self:lockRooms(pPalace)
end

function TheedPalace:getBuildingFaction(pPalace)
	local templatePath = SceneObject(pPalace):getTemplateObjectPath()
...
function TheedPalace:onEnterCorvette(pPalace, pPlayer)
	if not SceneObject(pPlayer):isPlayerCreature() then
		return 0
	end

	local active = readData("corvetteActive:" .. SceneObject(pPalace):getObjectID())
	if active ~= 1 then
		createEvent(10 * 1000, "TheedPalace", "handleNotAuthorized", pPlayer, "")
		return 0
	end

	local playerCount = readData("corvettePlayerCount:" .. SceneObject(pPalace):getObjectID())

	writeData("corvettePlayerCount:" .. SceneObject(pPalace):getObjectID(), playerCount + 1)

	if playerCount > 10 then
		createEvent(10 * 1000, "TheedPalace", "handleTooMany", pPlayer, "")
		return 0
	end
...
function TheedPalace:onExitCorvette(pPalace, pPlayer)
	if not SceneObject(pPlayer):isPlayerCreature() then
		return 0
	end
...
function TheedPalace:handleNotAuthorized(pPlayer)
...
function TheedPalace:handleTooMany(pPlayer)
...
function TheedPalace:handleCorvetteTimer(pPalace)
	local startTime = readData("corvetteStartTime:" .. SceneObject(pPalace):getObjectID())
	local timeLeftSecs = 3600 - (os.time() - startTime)
	local timeLeft = math.floor(timeLeftSecs / 60)

	if (timeLeft > 10) then
		self:broadcastToPlayers(pPalace, "@dungeon/corvette:timer_" .. timeLeft)
		createEvent(5 * 60 * 1000, "TheedPalace", "handleCorvetteTimer", pPalace, "")
	elseif (timeLeft >= 3) then
		self:broadcastToPlayers(pPalace, "@dungeon/corvette:timer_" .. timeLeft)
		createEvent(60 * 1000, "TheedPalace", "handleCorvetteTimer", pPalace, "")
	elseif (timeLeft >= 2) then
		self:broadcastToPlayers(pPalace, "@dungeon/corvette:timer_" .. timeLeft)
		createEvent(30 * 1000, "TheedPalace", "handleCorvetteTimer", pPalace, "")
	elseif (timeLeftSecs >= 90) then
		self:broadcastToPlayers(pPalace, "@dungeon/corvette:timer_90s")
		createEvent(30 * 1000, "TheedPalace", "handleCorvetteTimer", pPalace, "")
	elseif (timeLeftSecs >= 60) then
		self:broadcastToPlayers(pPalace, "@dungeon/corvette:timer_1")
		createEvent(30 * 1000, "TheedPalace", "handleCorvetteTimer", pPalace, "")
	elseif (timeLeftSecs >= 30) then
		self:broadcastToPlayers(pPalace, "@dungeon/corvette:timer_30s")
		createEvent(20 * 1000, "TheedPalace", "handleCorvetteTimer", pPalace, "")
	elseif (timeLeftSecs >= 10) then
		self:broadcastToPlayers(pPalace, "@dungeon/corvette:timer_10s")
		createEvent(10 * 1000, "TheedPalace", "handleCorvetteTimer", pPalace, "")
	else
		self:handleQuestFailure(pPalace)
	end
end

function TheedPalace:getCorvetteObject(pPlayer)
	if (pPlayer == nil) then
		return nil
	end
...
function TheedPalace:handleQuestFailure(pPalace)
	self:ejectAllPlayers(pPalace)
	createEvent(5000, "TheedPalace", "doCorvetteCleanup", pPalace, "")
end

function TheedPalace:handleQuestSuccess(pPalace)
	self:broadcastToPlayers(pPalace, "@dungeon/corvette:escape_pods") -- You have completed your assignment! Make your way to the escape pods before time expires and get off this ship!
	self:writeDataToGroup(pPalace, ":corvetteMissionComplete", 1)
	self:giveBadgeToGroup(pPalace)
end

function TheedPalace:giveBadgeToGroup(pPalace)
	local corvetteFaction = self:getBuildingFaction(pPalace)
	local questType = readStringData("corvetteQuestType:" .. SceneObject(pPalace):getObjectID())
	...
	for i = 1, 66, 1 do
		local pCell = BuildingObject(pPalace):getCell(i)
...
function TheedPalace:broadcastToPlayers(pPalace, message)
	for i = 1, 66, 1 do
		local pCell = BuildingObject(pPalace):getCell(i)
...
function TheedPalace:ejectAllPlayers(pPalace)
	...
	for i = 1, #playersToEject, 1 do
		...
		createEvent(1000, "TheedPalace", "ejectPlayer", pObject, "")
	end
end

function TheedPalace:writeDataToGroup(pPalace, key, data)
	for i = 1, 66, 1 do
		local pCell = BuildingObject(pPalace):getCell(i)
...
function TheedPalace:readDataFromGroup(pPalace, key)
	for i = 1, 66, 1 do
		local pCell = BuildingObject(pPalace):getCell(i)
...
function TheedPalace:increaseGroupFactionStanding(pPalace, faction, points)
	for i = 1, 66, 1 do
		local pCell = BuildingObject(pPalace):getCell(i)
...
function TheedPalace:checkSlicingSkill(pPlayer)
	if (pPlayer == nil) then
		return 0
	end
...
function TheedPalace:ejectPlayer(pPlayer)
	if pPlayer == nil then
		return
	end
	...
	local pPalace = SceneObject(pParent):getParent()

	if pPalace == nil or not SceneObject(pPalace):isBuildingObject() then
		printLuaError("TheedPalace:ejectPlayer unable to find corvette object")
		return
	end
	...
	if point == nil then
		printLuaError("TheedPalace:ejectPlayer was unable to grab an escape point for faction: " .. faction)
		return
	end

	local playerID = SceneObject(pPlayer):getObjectID()
	local ownerID = readData(SceneObject(pPalace):getObjectID() .. ":ownerID")
	...
	if (isZoneEnabled(point.planet)) then
		...
		printLuaError("TheedPalace:ejectPlayer attempted to eject a player to a zone that was not enabled.")
	end

	self:doPlayerCleanup(pPlayer)
end

function TheedPalace:doPlayerCleanup(pPlayer)
	if (pPlayer == nil) then
		return
	end
...
function TheedPalace:doCorvetteCleanup(pPalace)
	if (pPalace == nil) then
		return
	end
	...
	deleteData(palaceID .. ":fuelSetting")
	deleteData(palaceID .. ":engineSetting")
	deleteData(palaceID .. ":hyperdriveSetting")
	deleteData(palaceID .. ":engineDestroyed")
	deleteData(palaceID .. ":electricTrapEnabled")
	deleteData(palaceID .. ":H3P0ID")
	deleteData(palaceID .. ":repairDroidComplete")
	deleteData(palaceID .. ":ownerID")
	deleteData("corvetteActive:" .. palaceID)
	deleteData("corvetteDungeonID:" .. palaceID)
	deleteData("corvetteStartTime:" .. palaceID)
end

function TheedPalace:getNewDungeonID()
	local lastID = tonumber(getQuestStatus("TheedPalace:lastDungeonID"))
	local newID = 0
	...
	setQuestStatus("TheedPalace:lastDungeonID", newID)

	return newID
end

function TheedPalace:getCorvetteFromDungeonID(dungeonID)
	local palaceID = 0
	for i = 1, #self.buildings, 1 do
	...
	return getSceneObject(palaceID)
end
