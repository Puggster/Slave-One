...
PalaceTicketTakerLogic = ScreenPlay:new {
	numberOfActs = 1,
	npc = {},
	takerName = "PalaceTicketTakerLogic",
	faction = 0,
	goodbyeString = "",
	helpMeString = "",
	nevermindString = "",
...
function PalaceTicketTakerLogic:start()
	self:spawnNpc()
end

function PalaceTicketTakerLogic:spawnNpc()
...
function PalaceTicketTakerLogic:checkFaction(pPlayer)
...
function PalaceTicketTakerLogic:validateTicket(pPlayer)
	if (pPlayer == nil) then
		return
	end
...
function PalaceTicketTakerLogic:finishValidateTicket(pPlayer)
	if pPlayer == nil then
		return
	end
	...
	local result = TheedPalace:activate(pPlayer, self:getFactionString(), 1)

end

function PalaceTicketTakerLogic:getFactionString()
