			...
			if (cost ~= 0) then
				...
				if (currency == "credits") then
					if (cost <= CreatureObject(pPlayer):getCashCredits()) then
						...
						CreatureObject(pPlayer):subtractBankCredits(cost)
					end
				end
	...
	for i = containerSize - 1 , 0 , -1 do
		...
		if (invSceno:getTemplateObjectPath() == self.currencies[num].template and deleteItems > 0 ) then
			...
			elseif (invTano:getUseCount() - cost == 0) then
				...
				deleteItems = 0
				invTano:setUseCount(invTano:getUseCount() - cost)
				break
			end
		end
	end
