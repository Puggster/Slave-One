...
int ContainerComponent::canAddObject(SceneObject* sceneObject, SceneObject* object, int containmentType, String& errorDescription) const {
	...
	if (containmentType >= 4) {
		...
		if (object->getArrangementDescriptorSize() > arrangementGroup) {
			...
			//dw update
			//would segfault if it'd equip both slots while holding oh and oh not having been unequipped yet
			bool holdr = false;
			bool holdl = false;
				...
				if (holdl && holdr) {
					...
					if (creo != nullptr) {
						if (creo->getOffHandWeapon() != nullptr) {
							errorDescription = "Unequip your offhand to equip a weapon that uses both weapon slots.. or try again";
							return TransferErrorCode::CANTADD;
						}
					}
				}
